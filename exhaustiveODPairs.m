...
%% generated ODs
disp('2. Generate ODs...')
origin = zeros(length(node_id_eCBD)*length(node_id_eCBD) - length(node_id_eCBD),1);
dest = zeros(length(node_id_eCBD)*length(node_id_eCBD) - length(node_id_eCBD),1);

k = 1;
for i = 1 : length(node_id_eCBD)
    for j = 1 : length(node_id_eCBD)
        if (node_id_eCBD(i) ~= node_id_eCBD(j))
...
filenameB = sprintf('ODpairs_%d.csv', length(origin));
