...
for i = 1:length(trip_origin_node)
    ...
    if (cust_iter)
        ...
        % remove the trip
        toBeRemovedOrig = [toBeRemovedOrig, i];
    end
end

trip_origin_node (toBeRemovedOrig) = [];
trip_destination_node (toBeRemovedOrig) = [];
origin_x (toBeRemovedOrig) = [];
origin_y (toBeRemovedOrig) = [];
...
for i = 1:length(trip_destination_node)
    ...
    if (cust_iter)
        ...
        % remove the trip
        toBeRemovedDest = [toBeRemovedDest, i];
    end
    
end

trip_origin_node (toBeRemovedDest) = [];
trip_destination_node (toBeRemovedDest) = [];
dest_x (toBeRemovedDest) = [];
dest_y (toBeRemovedDest) = [];
origin_x (toBeRemovedDest) = [];
origin_y (toBeRemovedDest) = [];
